...
const getData = function (cliParams) {
  const db = new sqlite3.Database('./databases/olympic_history.db', (err) => {
    if (err) throw err;
  });

  return new Promise((resolve, reject) => {
    if (cliParams.chartName === 'medals') {
      const sql = getMedalsSql(cliParams);
      ...
      const sql = getTopTeamsSql(cliParams);
